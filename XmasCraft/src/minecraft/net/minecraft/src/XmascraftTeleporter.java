...
public class XmascraftTeleporter extends Teleporter
{
	...
	public boolean placeInExistingPortal(World world, Entity entity)
	{
		...
		for (int j1 = l - c; j1 <= l + c; j1++)
		{
			...
			for (int j2 = i1 - c; j2 <= i1 + c; j2++)
			{
				...
				{
					if (world.getBlockId(j1, k2, j2) != mod_Xmascraft.XmascraftPortal.blockID)
					{
						continue;
					}
					for (; world.getBlockId(j1, k2 - 1, j2) == mod_Xmascraft.XmascraftPortal.blockID; k2--) { }
		...
		if (d >= 0.0D)
		{
			...
			if (world.getBlockId(k1 - 1, l1, i2) == mod_Xmascraft.XmascraftPortal.blockID)
			{
				d2 -= 0.5D;
			}
			if (world.getBlockId(k1 + 1, l1, i2) == mod_Xmascraft.XmascraftPortal.blockID)
			...
			if (world.getBlockId(k1, l1, i2 - 1) == mod_Xmascraft.XmascraftPortal.blockID)
			{
				d6 -= 0.5D;
			}
			if (world.getBlockId(k1, l1, i2 + 1) == mod_Xmascraft.XmascraftPortal.blockID)
			...
			for (int i5 = -1; i5 <= 1; i5++)
			{
				for (int i6 = 1; i6 < 3; i6++)
				{
					for (int j7 = -1; j7 < 3; j7++)
					{
						...
						boolean flag = j7 < 0;
						world.setBlockWithNotify(k8, l9, i11, flag ? mod_Xmascraft.XmascraftStone.blockID : 0);
					}
				}
			}
		}
		for (int j5 = 0; j5 < 4; j5++)
		{
			...
			for (int j6 = 0; j6 < 4; j6++)
			{
				for (int k7 = -1; k7 < 4; k7++)
				{
					...
					world.setBlockWithNotify(l8, i10, j11, flag1 ? mod_Xmascraft.XmascraftStone.blockID : mod_Xmascraft.XmascraftPortal.blockID);
				}
			}
