...
public class WorldGenNorthpoleTrees extends WorldGenerator
{
    ...
    public WorldGenNorthpoleTrees(boolean par1)
    {
        this(par1, 4, 0, 0, false);
    }

    public WorldGenNorthpoleTrees(boolean par1, int par2, int par3, int par4, boolean par5)
    {
        super(par1);
        field_48202_a = par2;
        field_48201_c = par3;
    ...
    public boolean generate(World par1World, Random par2Random, int par3, int par4, int par5)
    {
            ...
            for (int l = par3 - byte0; l <= par3 + byte0 && flag; l++)
            {
                for (int j1 = par5 - byte0; j1 <= par5 + byte0 && flag; j1++)
                {
                    if (j >= 0 && j < 256)
                    {
                        ...
                        if (j2 != 0 && j2 != Block.leaves.blockID && j2 != mod_Xmascraft.XmascraftSnow.blockID && j2 != mod_Xmascraft.XmascraftSnow.blockID && j2 != Block.wood.blockID)
                        {
                            flag = false;
                        }
                    }
        ...
        if (k != mod_Xmascraft.XmascraftSnow.blockID && k != mod_Xmascraft.XmascraftSnow.blockID || par4 >= 256 - i - 1)
        {
            return false;
        }

        func_50073_a(par1World, par3, par4 - 1, par5, mod_Xmascraft.XmascraftSnow.blockID);
        byte byte1 = 3;
        int i1 = 0;
