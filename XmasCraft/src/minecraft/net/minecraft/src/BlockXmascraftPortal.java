...
public class BlockXmascraftPortal extends BlockPortalBase
{
	//Look in BlockPortalBase.java to see the full array of hooks you can use for your portal block.

	public BlockXmascraftPortal(int i)
	{
		super(i, ModLoader.addOverride("/terrain.png", "/terrain.png"), Material.portal);
	...
	{
		return new WorldProviderXmascraft();
	}

	public Teleporter getTeleporter()
	{
		return new XmascraftTeleporter();
	}
	...
	public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4)
	{
		...
		if (par1World.getBlockId(par2 - 1, par3, par4) == mod_Xmascraft.XmascraftIce.blockID || par1World.getBlockId(par2 + 1, par3, par4) == mod_Xmascraft.XmascraftStone.blockID)
		{
			i = 1;
		}

		if (par1World.getBlockId(par2, par3, par4 - 1) == mod_Xmascraft.XmascraftStone.blockID || par1World.getBlockId(par2, par3, par4 + 1) == mod_Xmascraft.XmascraftStone.blockID)
		{
			j = 1;
		}
		...
		for (int k = -1; k <= 2; k++)
		{
			for (int i1 = -1; i1 <= 3; i1++)
			{
				...
				if (flag)
				{
					if (k1 != mod_Xmascraft.XmascraftStone.blockID)
					{
						return false;
					}
		...
		for (int l = 0; l < 2; l++)
		{
			...
			{
				par1World.setBlockWithNotify(par2 + i * l, par3 + j1, par4 + j * l, mod_Xmascraft.XmascraftPortal.blockID);
			}
		}
	...
	public void onNeighborBlockChange(World par1World, int par2, int par3, int par4, int par5)
	{
		...
		if (par1World.getBlockId(par2, k - 1, par4) !=mod_Xmascraft.XmascraftStone.blockID)
		{
		...
		if (l != 3 || par1World.getBlockId(par2, k + l, par4) !=mod_Xmascraft.XmascraftStone.blockID)
		{
			par1World.setBlockWithNotify(par2, par3, par4, 0);
			return;
		}
		...
		if ((par1World.getBlockId(par2 + i, par3, par4 + j) != mod_Xmascraft.XmascraftStone.blockID || par1World.getBlockId(par2 - i, par3, par4 - j) != blockID) && (par1World.getBlockId(par2 - i, par3, par4 - j) != mod_Xmascraft.XmascraftStone.blockID || par1World.getBlockId(par2 + i, par3, par4 + j) != blockID))
		{
			par1World.setBlockWithNotify(par2, par3, par4, 0);
			return;
		}
