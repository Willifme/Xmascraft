    ...
    public boolean func_4106_b(World world, Entity entity)
    {
        ...
        for(int j1 = l - c; j1 <= l + c; j1++)
        {
            ...
            for(int j2 = i1 - c; j2 <= i1 + c; j2++)
            {
                ...
                {
                    if(world.func_600_a(j1, k2, j2) != portalID || portalMetaSensitivity && world.func_602_e(j1, k2, j2) != portalMetadata)
                    {
                        continue;
                    }
                    for(; world.func_600_a(j1, k2 - 1, j2) == portalID; k2--) { }
        ...
        if(d >= 0.0D)
        {
            ...
            if(world.func_600_a(k1 - 1, l1, i2) == portalID && (!portalMetaSensitivity || world.func_602_e(k1 - 1, l1, i2) == portalMetadata))
            {
                d2 -= 0.5D;
            }
            if(world.func_600_a(k1 + 1, l1, i2) == portalID && (!portalMetaSensitivity || world.func_602_e(k1 + 1, l1, i2) == portalMetadata))
            ...
            if(world.func_600_a(k1, l1, i2 - 1) == portalID && (!portalMetaSensitivity || world.func_602_e(k1, l1, i2 - 1) == portalMetadata))
            {
                d6 -= 0.5D;
            }
            if(world.func_600_a(k1, l1, i2 + 1) == portalID && (!portalMetaSensitivity || world.func_602_e(k1, l1, i2 + 1) == portalMetadata))
            ...
            for(int i5 = -1; i5 <= 1; i5++)
            {
                for(int i6 = 1; i6 < 3; i6++)
                {
                    for(int j7 = -1; j7 < 3; j7++)
                    {
                        ...
                        if(flag)
                        {
                            world.func_688_b(k8, l9, i11, frameID, frameMetadata);
                        } else
                        {
                            world.func_690_d(k8, l9, i11, 0);
                        }
                    }
        ...
        for(int j5 = 0; j5 < 4; j5++)
        {
            ...
            for(int j6 = 0; j6 < 4; j6++)
            {
                for(int k7 = -1; k7 < 4; k7++)
                {
                    ...
                    if(flag1)
                    {
                        world.func_688_b(l8, i10, j11, frameID, frameMetadata);
                    } else
                    {
                        world.func_688_b(l8, i10, j11, portalID, portalMetadata);
                    }
                }
