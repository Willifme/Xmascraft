...
//            Profiler, EnumSkyBlock, DimensionAPI, IBlockRemoval, 
//            BlockContainer, IBlockAdded, TileEntity, Entity, 
//            MathHelper, ChunkPosition, AxisAlignedBB, IChunkProvider, 
//            Material, ChunkCoordIntPair, NibbleArray, WorldChunkManager, 
//            BiomeGenBase
...
public class Chunk
{
        ...
        for(int l = 0; l < 16; l++)
        {
            for(int i1 = 0; i1 < 16; i1++)
            {
                for(int j1 = 0; j1 < k; j1++)
                {
                    ...
                    int l1 = j1 >> 4;
                    if(field_48505_p[l1] == null)
                    {
                        field_48505_p[l1] = new ExtendedBlockStorage(l1 << 4);
                    }
                    field_48505_p[l1].func_48691_a(l, j1 & 0xf, i1, k1);
                }
    ...
    {
        this(p_i14_1_, p_i14_2_, p_i14_3_, p_i14_4_, false);
    }
...
label0:
    ...
    public boolean func_1010_a(int p_1010_1_, int p_1010_2_, int p_1010_3_, int p_1010_4_, int p_1010_5_)
    {
        ...
        if(k != 0)
        {
            if(!field_1537_d.field_1026_y)
            {
                if(DimensionAPI.blockRemovalMap.containsKey(Integer.valueOf(k)))
                {
                    ...
                    iblockremoval.onBlockRemoval(field_1537_d, l, p_1010_2_, i1, k, field_1537_d.func_602_e(l, p_1010_2_, i1));
                } else
                {
        ...
        if(p_1010_4_ != 0)
        {
            if(!field_1537_d.field_1026_y)
            {
                if(DimensionAPI.blockAddedMap.containsKey(Integer.valueOf(p_1010_4_)))
                {
                    IBlockAdded iblockadded = (IBlockAdded)DimensionAPI.blockAddedMap.get(Integer.valueOf(p_1010_4_));
                    iblockadded.onBlockAdded(field_1537_d, l, p_1010_2_, i1, p_1010_4_, p_1010_5_);
                } else
                {
    ...
    public int func_35840_c(int p_35840_1_, int p_35840_2_)
    {
        ...
        if(j == -999)
        {
            ...
            for(j = -1; k > 0 && j == -1;)
            {
                ...
                Material material = l == 0 ? Material.field_1337_a : Block.field_345_n[l].field_356_bn;
